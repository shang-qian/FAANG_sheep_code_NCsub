...
{
tmp=pair_list[[i]]
geneID=unique(tmp[tmp$promoter %in% tissue_list[[i]]$V1,]$pair)
...
# 绘制到文件或显示
pdf(paste0(args[5],"/02",args[5],"_venn_5species_promoter.pdf"))
grid.draw(temp)
dev.off()
...
write.table(as.data.frame(Cerebellum_unique1), file=paste0(args[5],"/03",args[5],"_Cerebellum_uniq_Promoter.bed"), sep="\t", row.names=FALSE, quote=FALSE, col.names=F)
# 2. 找到Cortex的独特基因
Cortex_unique <- setdiff(Cortex, union(union(Cerebellum, Lung), Muscle))
Cortex_unique1=sub("gene-","",Cortex_unique)
write.table(as.data.frame(Cortex_unique1), file=paste0(args[5],"/04",args[5],"_Cortex_uniq_Promoter.bed"), sep="\t", row.names=FALSE, quote=FALSE, col.names=F)
# 3. 找到Lung的独特基因
Lung_unique <- setdiff(Lung, union(union(Cerebellum, Cortex), Muscle))
Lung_unique1=sub("gene-","",Lung_unique)
write.table(as.data.frame(Lung_unique1), file=paste0(args[5],"/05",args[5],"_Lung_uniq_Promoter.bed"), sep="\t", row.names=FALSE, quote=FALSE, col.names=F)
# 4. 找到Muscle的独特基因
Muscle_unique <- setdiff(Muscle, union(union(Cerebellum, Cortex), Lung))
Muscle_unique1=sub("gene-","",Muscle_unique)
write.table(as.data.frame(Muscle_unique1), file=paste0(args[5],"/06",args[5],"_Muscle_uniq_Promoter.bed"), sep="\t", row.names=FALSE, quote=FALSE, col.names=F)
...
write.table(as.data.frame(brain_unique1), file=paste0(args[5],"/07",args[5],"_Brain_uniq_Promoter.bed"), sep="\t", row.names=FALSE, quote=FALSE, col.names=F)


common_genes_all <- Reduce(intersect, list(Cerebellum, Cortex, Lung, Muscle))
common_genes_all1=sub("gene-","",common_genes_all)
write.table(as.data.frame(common_genes_all1), file=paste0(args[5],"/08",args[5],"_4tissues_common_Promoter.bed"), sep="\t", row.names=FALSE, quote=FALSE, col.names=F)


All_genes <- union(Cerebellum, union(union(Cortex, Lung), Muscle))
All_genes1=sub("gene-","",All_genes)
write.table(as.data.frame(All_genes1), file=paste0(args[5],"/09",args[5],"_4tissues_any1_Promoter.bed"), sep="\t", row.names=FALSE, quote=FALSE, col.names=F)
