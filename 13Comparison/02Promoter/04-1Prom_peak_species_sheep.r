...
#cattle
c_files=list.files(path =".", pattern = "*01cattle.*liftover8.promoter", full.names = TRUE, recursive = TRUE)
c_list <- lapply(c_files, read_peaks)
cattle=reduce(do.call(c, c_list))

#pig
p_files=list.files(path =".", pattern = "*01pig.*liftover8.promoter", full.names = TRUE, recursive = TRUE)
p_list <- lapply(p_files, read_peaks)
pig=reduce(do.call(c, p_list))

#mouse
m_files=list.files(path =".", pattern = "*01mouse.*liftover8.promoter", full.names = TRUE, recursive = TRUE)
m_list <- lapply(m_files, read_peaks)
mouse=reduce(do.call(c, m_list))

#human
h_files=list.files(path =".", pattern = "*01human.*liftover8.promoter", full.names = TRUE, recursive = TRUE)
...
write.table(as.data.frame(cattle_output), file=paste0(Tisname,"/02",Tisname,"cattle_peaks_Prom_sheep.bed1"), sep="\t", row.names=FALSE, quote=FALSE, col.names=TRUE)
write.table(as.data.frame(pig_output), file=paste0(Tisname,"/02",Tisname,"pig_peaks_Prom_sheep.bed1"), sep="\t", row.names=FALSE, quote=FALSE, col.names=TRUE)
write.table(as.data.frame(mouse_output), file=paste0(Tisname,"/02",Tisname,"mouse_peaks_Prom_sheep.bed1"), sep="\t", row.names=FALSE, quote=FALSE, col.names=TRUE)
write.table(as.data.frame(human_output), file=paste0(Tisname,"/02",Tisname,"human_peaks_Prom_sheep.bed1"), sep="\t", row.names=FALSE, quote=FALSE, col.names=TRUE)
...
write.table(as.data.frame(cs_output), file=paste0(Tisname,"/03",Tisname,"cattle_Prom_sheep.bed1"), sep="\t", row.names=FALSE, quote=FALSE, col.names=TRUE)
write.table(as.data.frame(ps_output), file=paste0(Tisname,"/03",Tisname,"pig_Prom_sheep.bed1"), sep="\t", row.names=FALSE, quote=FALSE, col.names=TRUE)
write.table(as.data.frame(ms_output), file=paste0(Tisname,"/03",Tisname,"mouse_Prom_sheep.bed1"), sep="\t", row.names=FALSE, quote=FALSE, col.names=TRUE)
write.table(as.data.frame(hs_output), file=paste0(Tisname,"/03",Tisname,"human_Prom_sheep.bed1"), sep="\t", row.names=FALSE, quote=FALSE, col.names=TRUE)
...
# Plot bar chart
pdf(paste0(Tisname,"/04",Tisname,"_barplot_5species_promoter.pdf"))
bar_positions=barplot(range_counts, names.arg = dataset_names, col = "red",
        main = "Number of enhancers identified in Each Dataset", xlab = "Dataset", ylab = "Promoter count", ylim=c(0,max(range_counts)+1000))
...
# 生成并保存 UpSet 图
pdf(paste0(Tisname,"/05",Tisname,"_species_upset_plot_promoter.pdf") ) # 保存为 PDF 文件
upset(upset_data, sets = colnames(overlap_matrix), 
      main.bar.color = "red", sets.bar.color = "red", 
      order.by = "freq", keep.order = TRUE,
      sets.x.label = "Number of Promoters")
dev.off()
...
write.table(as.data.frame(sheep_uniq), file=paste0(Tisname,"/06",Tisname,"_sheep_uniq_promoter.bed"), sep="\t", row.names=FALSE, quote=FALSE, col.names=TRUE)
...
ruminant_uniq <- rum3
write.table(as.data.frame(ruminant_uniq), file=paste0(Tisname,"/07",Tisname,"_sheep_ruminant_uniq_promoter.bed"), sep="\t", row.names=FALSE, quote=FALSE, col.names=TRUE)


####sheep_common5
common_uniq <- com5
write.table(as.data.frame(common_uniq), file=paste0(Tisname,"/08",Tisname,"_sheep_common5_uniq_promoter.bed"), sep="\t", row.names=FALSE, quote=FALSE, col.names=TRUE)
